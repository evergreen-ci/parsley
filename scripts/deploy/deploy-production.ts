...
const localDeploy = async () => {
  const response = await prompts({
    ...
    name: "value",
    message:
      "Are you sure you'd like to build Parsley locally and push directly to S3? This is a high-risk operation that requires a correctly configured local environment.",
  });
